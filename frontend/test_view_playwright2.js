...
(async () => {
  ...
  // Login
  const email = 'test_view2_' + Date.now() + '@example.com';
  ...
  await Promise.all([
    ...
    page.click('text="View Goal"')
  ]);
  
  await page.waitForTimeout(2000);
  console.log("Current URL after click:", page.url());
  const content = await page.content();
  console.log(content.includes('Test Goal API') ? 'PAGE LOADED CORRECTLY' : 'PAGE IS BROKEN');
  if (!content.includes('Test Goal API')) {
    console.log(content);
  }
